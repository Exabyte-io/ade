...
`github:mat3ra/ade#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/ade/releases/download/wip-<short-sha>/ade.tgz
```
...
```bash
npm run mat3ra:install -- ade wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/ade@<release-url>` with
